...
class DualCache(BaseCache):
    ...
    def set_cache(self, key, value, local_only: bool = False, **kwargs):
        ...
        try:
            if self.in_memory_cache is not None or (self.redis_cache is not None and not local_only):
                # Reuse kwargs dict—avoid accidental mutation of caller dict
                kwargs_for_in_memory = dict(kwargs)
                if self.in_memory_cache is not None:
                    if "ttl" not in kwargs_for_in_memory and self.default_in_memory_ttl is not None:
                        kwargs_for_in_memory["ttl"] = self.default_in_memory_ttl
                    self.in_memory_cache.set_cache(key, value, **kwargs_for_in_memory)

                if self.redis_cache is not None and not local_only:
