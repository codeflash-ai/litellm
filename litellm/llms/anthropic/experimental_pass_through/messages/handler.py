...
async def anthropic_messages(
    ...
    **kwargs,
    ...
    response = await _anthropic_messages_handler_async(
...
async def _anthropic_messages_handler_async(
    max_tokens: int,
    messages: List[Dict],
    model: str,
    metadata: Optional[Dict] = None,
    ...
    **kwargs,
    ...
    """
    Async Handler for Anthropic `/v1/messages` API calls In the Anthropic API Spec.
    Optimized to avoid blocking via run_in_executor; directly runs async logic.
    """
    from litellm.types.utils import LlmProviders
    ...
    if anthropic_messages_provider_config is None:
        ...
        # Await async handler if _is_async is True (branch is only for async calls here)
        handler_result = LiteLLMMessagesToCompletionTransformationHandler.anthropic_messages_handler(
            max_tokens=max_tokens,
            messages=messages,
            model=model,
            metadata=metadata,
    ...
    # NOTE: If base_llm_http_handler implements async handler, await if needed
    http_handler_result = base_llm_http_handler.anthropic_messages_handler(
        model=model,
        messages=messages,
        anthropic_messages_provider_config=anthropic_messages_provider_config,
        anthropic_messages_optional_request_params=dict(anthropic_messages_optional_request_params),
    ...
    if asyncio.iscoroutine(http_handler_result):
        return await http_handler_result

    return http_handler_result
