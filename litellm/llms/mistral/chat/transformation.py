...
class MistralConfig(OpenAIGPTConfig):
    ...
    def __init__(
        ...
        # Assign provided values to instance only when they are not None
        # Using __dict__ update to avoid attribute lookup and setattr loop overhead
        for key, value in (
    ...
    def _handle_name_in_message(cls, message: AllMessageValues) -> AllMessageValues:
        ...
        # Inline the checks to minimize branching and unnecessary pop operations
        if _name is not None:
            role = message["role"]
            if role != "tool" or (isinstance(_name, str) and not _name.strip()):
