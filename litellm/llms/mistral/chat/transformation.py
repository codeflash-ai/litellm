...
class MistralConfig(OpenAIGPTConfig):
    ...
    def _is_empty_assistant_message(cls, message: AllMessageValues) -> bool:
        ...
        """
        if not hasattr(cls, "_mistral__assistant_message_keys"):
            from litellm.types.llms.openai import ChatCompletionAssistantMessage

            cls._mistral__assistant_message_keys = tuple(
                get_type_hints(ChatCompletionAssistantMessage).keys()
            )

        set_keys = cls._mistral__assistant_message_keys
