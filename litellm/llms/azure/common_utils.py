...
class BaseAzureLLM(BaseOpenAILLM):
    ...
    def get_azure_openai_client(
        ...
        # Build client_initialization_params without using locals() for efficiency
        client_initialization_params = {
            ...
            "_is_async": _is_async,
            "model": model,
            "is_async": _is_async,  # For backward compatibility (used by cache key)
        }
        ...
        if client is None:
            ...
            if cached_client:
                if isinstance(cached_client, (AzureOpenAI, AsyncAzureOpenAI)):
            ...
            # Only cache if we created a new client here, not for user-passed clients
            ...
            if api_version is not None and isinstance(
                getattr(openai_client, "_custom_query", None), dict
            ):
