...
def _apply_cost_discount(
    ...
    # Move string interpolation outside of logger.debug for performance (compose one final string)
    if custom_llm_provider:
        # Avoid repeated lookups
        config = litellm.cost_discount_config
        percent = config.get(custom_llm_provider)
        if percent is not None:
            discount_percent = percent
            discount_amount = original_cost * discount_percent
            final_cost = original_cost - discount_amount

            # Build full message string using local variables (avoid repeated f-string computation)
            # This cuts back on expensive string formatting when verbose_logger.debug is disabled.
            if verbose_logger.isEnabledFor(getattr(verbose_logger, 'DEBUG', 10)):
                message = (
