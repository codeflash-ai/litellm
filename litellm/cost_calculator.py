...
def default_image_cost_calculator(
    ...
    # Avoid redundant replace if already in "-x-" format
    if "-x-" not in size_str and "x" in size_str:
        size_str = size_str.replace("x", "-x-")

    # Use partition instead of split for known exact formatting and safe unpacking
    h, _, w = size_str.partition("-x-")
    try:
        height = int(h)
        width = int(w)
    except ValueError:
        # Fallback for split (if partition fails for some reason), original logic fallback
    ...
    # Precompute split results
    model_split = model.split('/')
    model_without_provider = f"{size_str}/{model_split[-1]}"
    ...
    # Inline cost_info lookup (short-circuit upon first hit in model_cost)
    model_cost = litellm.model_cost
    for _model in models_to_check:
        if _model is not None:
            cost_info = model_cost.get(_model)
            if cost_info is not None:
                # Fast exit upon finding cost_info
