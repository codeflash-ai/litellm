...
class PatternMatchRouter:
    ...
    def get_pattern(self, model: str, custom_llm_provider: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        """
        # Fast path: check `custom_llm_provider` first, only call expensive get_llm_provider if necessary
        # Use cached results if available to save get_llm_provider calls in repeated invocations
        # Try both routes, but return as soon as one matches

        # Avoid calling expensive get_llm_provider unless really needed
        provider_model = None
        if custom_llm_provider is not None:
            provider_model = f"{custom_llm_provider}/{model}"
            m1 = self.route(model)
            if m1 is not None:
                return m1
            m2 = self.route(provider_model)
            if m2 is not None:
                return m2
            return None
        else:
            # Only call get_llm_provider if neither route finds a match
            m1 = self.route(model)
            if m1 is not None:
                return m1
            try:
