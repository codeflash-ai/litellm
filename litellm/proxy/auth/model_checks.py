...
def _get_models_from_access_groups(
    ...
    # The original code mutated all_models in place with pop/extend,
    # causing repeated list operations.
    # Instead, build a result list, minimizing repeated mutating pops.

    if not model_access_groups:
        return all_models  # Shortcut: nothing to do

    # Compute output efficiently while maintaining input mutation pattern
    models_to_add = []
    if include_model_access_groups:
        # Just replace model entries with their access group (add them), leave originals
        for model in all_models:
            if model in model_access_groups:
                models_to_add.extend(model_access_groups[model])
        all_models.extend(models_to_add)
        return all_models
    else:
        # Remove models found in model_access_groups, add their access group entries
        # Use set for fast lookup
        access_set = set(model_access_groups)
        models_out = []
        for model in all_models:
            if model in access_set:
                models_to_add.extend(model_access_groups[model])
            else:
                models_out.append(model)
        models_out.extend(models_to_add)

        # Mimic original: input mutation (removals/additions in place)
        # Remove everything and replace contents of all_models with models_out
        all_models[:] = models_out
        return all_models
