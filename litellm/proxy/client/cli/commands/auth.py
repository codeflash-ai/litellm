...
def prompt_team_selection_fallback(teams: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    while True:
        try:
            ...
            # Fast path validation: only proceed if input looks like a number within the valid range
            if choice.isdigit():
                index = int(choice) - 1
                if 0 <= index < len_teams:
                    selected_team = teams[index]
                    click.echo(f"\n✅ Selected team: {selected_team.get('team_alias', 'N/A')} ({selected_team.get('team_id')})")
                    return selected_team
                else:
                    click.echo(f"❌ Invalid selection. Please enter a number between 1 and {len_teams}")
            else:
                click.echo("❌ Invalid input. Please enter a number or 'skip'")
        except KeyboardInterrupt:
            # KeyboardInterrupt is typically not a control-flow "error"; catch and return early as before.
            click.echo("\n❌ Team selection cancelled.")
            return None
