...
class ProxyLogging:
    ...
    async def pre_call_hook(
        ...
        if (
            ...
            custom_logger = IN_MEMORY_PROMPT_REGISTRY.get_prompt_callback_by_id(prompt_id)
        ...
        try:
            for callback in litellm.callbacks:
                ...
                if _callback is not None and isinstance(_callback, CustomGuardrail):
                    ...
                    if _callback.should_run_guardrail(data=data, event_type=event_type) is not True:
                        continue
