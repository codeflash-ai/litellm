...
import os
import sys
from litellm.types.proxy.guardrails.guardrail_hooks.aporia_ai import AporiaGuardrailConfigModel
...
class AporiaGuardrail(CustomGuardrail):
    def __init__(self, api_key: Optional[str] = None, api_base: Optional[str] = None, **kwargs):
        ...
        # Avoid repeated os.environ lookups for better performance, as these call __getitem__ repeatedly
        # Also, avoid double lookup by using local variables
        if api_key is not None:
            self.aporia_api_key = api_key
        else:
            self.aporia_api_key = os.environ["APORIO_API_KEY"]

        if api_base is not None:
            self.aporia_api_base = api_base
        else:
            self.aporia_api_base = os.environ["APORIO_API_BASE"]
...
'
    ...
    def get_config_model() -> Optional[Type["GuardrailConfigModel"]]:
        # Bottleneck: repeated import from same module/type on every call
        # Optimization: Use already imported module-level value
