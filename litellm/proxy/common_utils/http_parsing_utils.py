...
def _safe_set_request_parsed_body(
    ...
    # Fast path: exit early if request is None (avoid try/except for common case)
    if request is None:
        return
    try:
        # Directly assign keys as tuple (unchanged)
        request.scope["parsed_body"] = (tuple(parsed_body.keys()), parsed_body)
    except Exception as e:
        # f-string is faster than .format(), and preferred style since Python 3.6
        verbose_proxy_logger.debug(
            f"Unexpected error setting request parsed body - {e}"
        )
