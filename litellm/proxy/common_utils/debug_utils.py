...
def _get_uncollectable_objects_info() -> Dict[str, Any]:
    ...
    return {
        "count": count,
        "sample_types": [type(obj).__name__ for obj in uncollectable[:10]] if count > 0 else [],
        "warning": "If count > 0, you may have reference cycles preventing garbage collection" if count > 0 else None,
    }
