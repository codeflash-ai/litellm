...
def _get_cache_memory_stats(user_api_key_cache, llm_router, proxy_logging_obj, redis_usage_cache) -> Dict[str, Any]:
    ...
    try:
        # User API key cache
        user_cache = user_api_key_cache.in_memory_cache
        user_cache_dict = user_cache.cache_dict
        user_ttl_dict = user_cache.ttl_dict
        user_cache_size = sys.getsizeof(user_cache_dict)
        user_ttl_size = sys.getsizeof(user_ttl_dict)
        cache_stats["user_api_key_cache"] = {
            "num_items": len(user_cache_dict),
        ...
        # Router cache
        if llm_router is not None:
            router_inmem_cache = llm_router.cache.in_memory_cache
            router_cache_dict = router_inmem_cache.cache_dict
            router_ttl_dict = router_inmem_cache.ttl_dict
            router_cache_size = sys.getsizeof(router_cache_dict)
            router_ttl_size = sys.getsizeof(router_ttl_dict)
            cache_stats["llm_router_cache"] = {
                "num_items": len(router_cache_dict),
        ...
        # Proxy logging cache
        logging_cache = proxy_logging_obj.internal_usage_cache.dual_cache.in_memory_cache
        logging_cache_dict = logging_cache.cache_dict
        logging_ttl_dict = logging_cache.ttl_dict
        logging_cache_size = sys.getsizeof(logging_cache_dict)
        logging_ttl_size = sys.getsizeof(logging_ttl_dict)
        cache_stats["proxy_logging_cache"] = {
            "num_items": len(logging_cache_dict),
        ...
        if redis_usage_cache is not None:
            ...
            redis_client = getattr(redis_usage_cache, "redis_client", None)
            if redis_client is not None:
                pool_info = getattr(redis_client, "connection_pool", None)
                if pool_info is not None:
                    max_connections = getattr(pool_info, "max_connections", None)
                    connection_class = getattr(pool_info, "connection_class", None)
                    redis_cache_entry["connection_pool"] = {
                        "max_connections": max_connections,
                        "connection_class": getattr(connection_class, "__name__", None)
                        if connection_class is not None
                        else None,
                    }
            cache_stats["redis_usage_cache"] = redis_cache_entry
        else:
