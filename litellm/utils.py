    ...
    try:
        ...
        if kwargs.get("success_callback", None) is not None and isinstance(kwargs["success_callback"], list):
            ...
            for index, callback in enumerate(kwargs["success_callback"]):
                if get_coroutine_checker().is_async_callable(callback) or callback == "dynamodb" or callback == "s3":
...
def _return_huggingface_tokenizer(model: str) -> Optional[SelectTokenizerResponse]:
    if model in litellm.cohere_models and "command-r" in model:
        ...
        if cache_key not in _hf_tokenizer_cache:
            _hf_tokenizer_cache[cache_key] = Tokenizer.from_pretrained("Xenova/c4ai-command-r-v01-tokenizer")
...
def _add_path_to_api_base(api_base: str, ending_path: str) -> str:
    ...
    return new_url_wo_query + (f"?{parsed.query}" if parsed.query else "")
