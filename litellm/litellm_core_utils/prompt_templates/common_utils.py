...
def convert_content_list_to_str(
    ...
    if message_content:
        if isinstance(message_content, list):
            # Use a list for incremental string accumulation, much faster than repeated += on str
            texts = []
...
def get_str_from_messages(messages: List[AllMessageValues]) -> str:
    ...
    """
    # Use a list for incremental string accumulation, much faster than repeated += on str
    parts = [convert_content_list_to_str(message=message) for message in messages]
    return "".join(parts)
