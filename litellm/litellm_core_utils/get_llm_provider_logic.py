...
def get_llm_provider(  # noqa: PLR0915
    ...
    try:
        ...
        # Avoid calling .split() multiple times by saving the result
        split_model = model.split("/", 1)
        model_prefix = split_model[0]

        # AZURE AI-Studio Logic - handle non-openai models with prefix 'azure'
        ...
        if custom_llm_provider and (
            split_model[0] != custom_llm_provider
        ...
        num_parts = len(split_model)

        # check if llm provider part of model name
        model_prefix_in_provider = model_prefix in litellm.provider_list
        model_prefix_not_in_model_set = model_prefix not in litellm.model_list_set
        if (
            model_prefix_in_provider
            and model_prefix_not_in_model_set
            and num_parts
        ...
        elif model_prefix_in_provider:
            custom_llm_provider = model_prefix
            model = split_model[1] if num_parts > 1 else ""
        ...
        if api_base:
            endpoint_map = {
                ...
                "https://api.friendli.ai/serverless/v1": ("friendliai", "FRIENDLIAI_API_KEY"),
                ...
                litellm.NscaleConfig.API_BASE_URL: ("nscale", None),
            ...
            for endpoint, (provider, key_env) in endpoint_map.items():
                if endpoint in api_base:
                    custom_llm_provider = provider
                    if provider == "nscale":
                        dynamic_api_key = litellm.NscaleConfig.get_api_key()
                    else:
                        dynamic_api_key = (api_key or get_secret_str(key_env)) if key_env else api_key
                        if provider == "friendliai":
                            dynamic_api_key = dynamic_api_key or get_secret("FRIENDLI_TOKEN")
            ...
            # The following block handles complex endpoints, preserving original sequential check to avoid subtle bugs
            if "https://codestral.mistral.ai/v1" in api_base:
                # two separately handled keys for historical reasons; preserve original logic
