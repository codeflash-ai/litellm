...
def get_llm_provider(  # noqa: PLR0915
    ...
    try:
        ...
        # Hot path: cache splits and reuse
        if "/" in model:
            model_split_1 = model.split("/", 1)
            model_prefix = model_split_1[0]
            model_suffix = model_split_1[1]
        else:
            model_split_1 = [model]
            model_prefix = model
            model_suffix = ""
        ...
        # Only need to check again if '/' in model
        if custom_llm_provider and ("/" in model and model_prefix != custom_llm_provider):
        ...
        # Openai-compatible provider logic -- optimize membership checks via prebuilt mapping if possible
        if "/" in model:
            model_prefix_check = model.split("/", 1)[0]
        else:
            model_prefix_check = model

        if (
            model_prefix_check in litellm.provider_list
            and model_prefix_check not in litellm.model_list_set
            and len(model.split("/"))
        ...
        elif model_prefix_check in litellm.provider_list:
            # Split only once
        ...
        if api_base:
            # Define known endpoint substrings and provider/config mapping upfront (single dict)
            endpoint_map = {
                "api.perplexity.ai": ("perplexity", "PERPLEXITYAI_API_KEY"),
                "api.endpoints.anyscale.com/v1": ("anyscale", "ANYSCALE_API_KEY"),
                "api.deepinfra.com/v1/openai": ("deepinfra", "DEEPINFRA_API_KEY"),
                "api.mistral.ai/v1": ("mistral", "MISTRAL_API_KEY"),
                "api.groq.com/openai/v1": ("groq", "GROQ_API_KEY"),
                "https://integrate.api.nvidia.com/v1": ("nvidia_nim", "NVIDIA_NIM_API_KEY"),
                "https://api.cerebras.ai/v1": ("cerebras", "CEREBRAS_API_KEY"),
                "https://inference.baseten.co/v1": ("baseten", "BASETEN_API_KEY"),
                "https://api.sambanova.ai/v1": ("sambanova", "SAMBANOVA_API_KEY"),
                "https://api.ai21.com/studio/v1": ("ai21_chat", "AI21_API_KEY"),
                "https://codestral.mistral.ai/v1": ("codestral", "CODESTRAL_API_KEY"),
                "app.empower.dev/api/v1": ("empower", "EMPOWER_API_KEY"),
                "api.deepseek.com/v1": ("deepseek", "DEEPSEEK_API_KEY"),
                "https://api.friendli.ai/serverless/v1": ("friendliai", ["FRIENDLIAI_API_KEY", "FRIENDLI_TOKEN"]),
                "api.galadriel.com/v1": ("galadriel", "GALADRIEL_API_KEY"),
                "https://api.llama.com/compat/v1": ("meta_llama", "LLAMA_API_KEY"),
                "https://api.featherless.ai/v1": ("featherless_ai", "FEATHERLESS_AI_API_KEY"),
                getattr(litellm.NscaleConfig, "API_BASE_URL", None): ("nscale", "NSCALE_API_KEY"),
                "dashscope-intl.aliyuncs.com/compatible-mode/v1": ("dashscope", "DASHSCOPE_API_KEY"),
                "api.moonshot.ai/v1": ("moonshot", "MOONSHOT_API_KEY"),
                "https://api.v0.dev/v1": ("v0", "V0_API_KEY"),
                "https://api.lambda.ai/v1": ("lambda_ai", "LAMBDA_API_KEY"),
                "https://api.hyperbolic.xyz/v1": ("hyperbolic", "HYPERBOLIC_API_KEY"),
                "https://ai-gateway.vercel.sh/v1": ("vercel_ai_gateway", "VERCEL_AI_GATEWAY_API_KEY"),
                "https://api.inference.wandb.ai/v1": ("wandb", "WANDB_API_KEY"),
            }
            found_endpoint = None
            found_provider = None
            key = None

            # Use substring matching for endpoints as before, but no loop over entire list
            for endpoint_sub, provider_info in endpoint_map.items():
                if endpoint_sub and endpoint_sub in api_base:
                    found_endpoint = endpoint_sub
                    found_provider, key = provider_info
                    break

            if found_endpoint:
                custom_llm_provider = found_provider
                # Some keys require fallback via get_secret or get_secret_str etc.
                if isinstance(key, list):
                    dynamic_api_key = None
                    for k in key:
                        dynamic_api_key = get_secret_str(k) or get_secret(k)
                        if dynamic_api_key:
                            break
                elif found_provider == "meta_llama":  # this one can use api_key directly
                    dynamic_api_key = api_key or get_secret_str(key)
                elif found_provider == "nscale" and hasattr(litellm, "NscaleConfig"):
                    # nscale unique logic (call NscaleConfig)
                    dynamic_api_key = getattr(litellm.NscaleConfig, "get_api_key", lambda: None)()
                else:
                    dynamic_api_key = get_secret_str(key)
